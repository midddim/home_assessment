...
test.describe("Home page", () => {
  test('has title @6d15bd09', {tag: '@smoke'}, async ({ homePage, page }) => {
  ...
  test('search for courses on a specific topic @a8a322cf', {tag: '@smoke'}, async ({ homePage, page }) => {
  ...
  test('search for courses on a specific topic with additional params @d8924fb0', {tag: '@smoke'}, async ({ homePage, page }) => {
    const searchQuery: string = 'javascript';
  ...
  test('search for courses with "No Results Found" result @50fbe391', {tag: '@smoke'}, async ({ homePage, page }) => {
